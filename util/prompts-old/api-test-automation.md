...
Read **all** application details from `util/prompts/aut-config.md` Section 2:
- API name, base URL, documentation URL
- Auth mechanism (token-based / Basic Auth)
- Endpoint list and known behaviours

Do not hardcode any URL, credential, or endpoint in this prompt or in generated files.
...
```typescript
// Read defaults from aut-config.md Section 2 — always use env vars with fallback
...
```typescript
// API-Tests project — must be present
{
  name: 'API-Tests',
...
- Never use `console.log` — use `test.step()` so steps appear in the trace viewer and HTML report
- Never use `if (status === 200) { ... }` conditional assertions — always assert explicitly with `expect`
- Always clean up resources created during a test (delete bookings) in `test.afterEach` using a `try/finally` block so cleanup runs even when an assertion fails
- Never hardcode credentials or base URLs — read from `process.env.API_USERNAME`, `process.env.API_PASSWORD`, `process.env.API_BASE_URL` with sensible defaults for local dev
- Never use `any` types — use proper TypeScript interfaces and generics throughout
- `getToken()` must throw a descriptive error if the response does not contain a token
- Use `generateFutureDates()` for all booking date values — never hardcode past dates
...
### 2. Use test.step() for Lifecycle Logging
```typescript
await test.step('Generate auth token', async () => {
  const response = await authService.createToken(VALID_AUTH);
  expect(response.status()).toBe(200);
  authToken = await authService.getToken(response);
});

await test.step('Create booking', async () => {
  const response = await bookingService.createBooking(COMPLETE_BOOKING);
  expect(response.status()).toBe(200);
  bookingId = await bookingService.getBookingIdFromResponse(response);
});
```

### 3. Always clean up created resources
```typescript
...
test('TC-AUTH-001: Generate auth token with valid credentials', async () => {
  await test.step('Create token with valid credentials', async () => {
    const response = await authService.createToken(VALID_AUTH);
...
### Example: CRUD Test with cleanup
```typescript
let createdBookingId: number | undefined;
...
test.afterEach(async ({ request }) => {
  if (createdBookingId) {
    ...
    const tokenResp = await auth.createToken(VALID_AUTH);
    const headers = auth.createAuthCookie(await auth.getToken(tokenResp));
    try { await svc.deleteBooking(createdBookingId, headers); }
    finally { createdBookingId = undefined; }
  }
});
...
### Example: E2E Test with test.step() and try/finally cleanup
```typescript
  ...
  try {
    ...
    await test.step('Update booking', async () => {
      const updateResponse = await bookingService.partialUpdateBooking(
        bookingId!, { firstname: 'Updated' }, authHeaders
      );
    ...
    await test.step('Verify deletion returns 404', async () => {
      const verifyResponse = await bookingService.getBookingById(bookingId ?? 0);
      expect(verifyResponse.status()).toBe(404);
    });
  } finally {
    // Safety net: delete booking if test failed before the delete step
    if (bookingId) {
