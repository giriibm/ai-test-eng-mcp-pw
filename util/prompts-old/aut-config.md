...
> **Change `mode` to switch between a quick demo suite and the full regression suite.**
> This is the only field you need to change — no other file needs to be edited.
...
| Mode | Behaviour |
|---|---|
| `demo` | Generate ONLY the scenarios listed in the Demo Scope table below — fast to run, good for demos and CI smoke |
| `full` | Generate ALL scenarios derived from the user story — full regression coverage |

---

### Demo Scope

When `mode: demo`, each prompt generates only the scenarios below.

#### UI Tests — 5 scenarios (Parts 1 → 4)

| # | Category | Scenario description |
|---|---|---|
| 1 | Happy Path | Complete checkout end-to-end (login → add product → cart → fill form → review → confirm) |
| 2 | Negative | Submit checkout form with all fields empty |
| 3 | Edge Case | Cart with exactly one product — verify badge count |
...
|---|---|---|
| 1 | `auth.spec.ts` | TC-AUTH-001: token generation with valid credentials |
| 2 | `create-booking.spec.ts` | TC-BOOKING-001: create booking with all fields (happy path) |
| 3 | `complete-lifecycle.spec.ts` | TC-E2E-001: create → retrieve → update → delete |

#### BDD Tests — 3 scenarios (bdd-test-automation-prompt)

The BDD suite is already a minimal smoke scope — generate all 3 scenarios in both `demo` and `full` mode (no change needed).
