...
| **Part 4 — this file** | `ui-step4-test-generation.md` | playwright-test-generator | Steps 4–5 | Test data JSON + POM automation scripts |
...
## STEP 3.5 — Create playwright.config.ts

If `playwright.config.ts` does not exist, create it now with the full configuration below. Read the API base URL default from `util/prompts/aut-config.md` Section 2 and substitute it for `<API_BASE_URL>`.

> This file can be safely deleted and regenerated by re-running Part 4 of this pipeline.
...
// Playwright-BDD: generate Playwright tests from Gherkin feature files
const testDir = defineBddConfig({
  paths: ['tests/bdd/features/**/*.feature'],
  require: ['tests/bdd/steps/**/*.ts'],
});

// Timestamp for this run — each run gets its own folder so results are never overwritten
const ts = new Date()
  .toISOString()
  .replace('T', '_')
  .replace(/:/g, '-')
...
export default defineConfig({
  ...
  projects: [
    // BDD smoke tests — runs Gherkin-generated specs
    {
      name: 'BDD-Smoke',
    ...
    // UI end-to-end tests
    {
      ...
      use: {
        ...devices['Desktop Chrome'],
        baseURL: undefined,  // UI tests navigate directly; baseURL is for API
      },
    },
    // API tests
    {
...
> The `credentials` block values must be read from `util/prompts/aut-config.md` Section 1. Do not hardcode them here — substitute the actual defaults from that file into the JSON below.

```json
{
  "credentials": {
    "username": "<UI_USERNAME from aut-config.md>",
    "password": "<UI_PASSWORD from aut-config.md>"
  },
  "products": {
...
test.describe('<Scenario Group Name>', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await loginPage.navigate(process.env.UI_BASE_URL ?? 'https://www.saucedemo.com');
...
- **Never call `page.screenshot()` inside spec files.** The global config captures screenshots for every test automatically. Use `captureScreenshot()` from BasePage only when a specific labelled evidence screenshot is required.
- **Each `test.describe` block must have a name that is unique across the entire spec suite.** Do not reuse names like `'Negative — Validation Errors'` across multiple files. Use the scenario ID as a suffix, e.g. `'Negative — Validation Errors (NE-06)'`.
- **Never hardcode the application URL in spec files or page objects.** Use `process.env.UI_BASE_URL ?? 'https://www.saucedemo.com'` in `BasePage.navigate()` and `LoginPage`.
- **Page objects must not contain boolean-returning visibility helpers** (`isVisible()`, `isEnabled()` etc.). Visibility assertions belong in specs only, using `expect(locator).toBeVisible()`.
- **Negative scenarios with no validation error must assert the correct outcome page, not a tautological URL check.** For `negativeSpecialChars` and `negativeLongStrings`, assert that the order overview page (`/checkout-step-two.html`) loaded successfully and that key elements (finish button, item total) are visible. The current SauceDemo behaviour is to accept these inputs and advance — so the correct assertion is that step two renders, not that the URL is one of two options.
...
Confirm every item below before opening `ui-step5-self-heal-report.md`.
